    ...
    try {
      ...
      if (result.rows.length > 0) {
        ...
        const pepper = process.env.PEPPER
          ? Buffer.from(pepperString, "utf-8")
          : undefined;
...
async function validateQuery(req, res, next) {
  try {
    ...
    if (direction === null) {
      direction2 = order === "mean" ? "desc" : "asc";
    }

    console.log("direction 2: ", direction2);
    direction2 =
      typeof direction2 === "string" && direction2.toUpperCase() === "DESC"
        ? direction2.toUpperCase()
        : "ASC";
    const order2 =
      typeof order === "string" && validOrder.has(order) ? order : "popularity";
    console.log("direction 3: ", direction2);
    ...
    console.log("direction:99 ", direction2);
...
async function sortMangaByFilters(filters, { page, limit, order, direction }) {
  try {
    ...
    for (let category in filters) {
      if (categories.includes(category)) {
        ...
        if (categoryTags.length > 0) {
          ...
          whereClause.push(
            ` (g.type='${category}' AND g.name= ANY($${queryIndex}))`,
          );
    ...
    let categoryType = Array.isArray(filters.type)
      ? filters.type
      : [filters.type];
...
app.post("/user/signup", async (req, res) => {
  try {
    ...
    if (!username || !passport || !email) {
      return res
        .status(400)
        .json({ success: false, error: "You MUST fill all fields." });
    }
    ...
    if (emailExists.rows[0].email_exists) {
      return res.status(400).json({
        success: false,
        error: "Email already exist, try a different one.",
      });
    }
    ...
    if (!upperCasePattern.test(password)) {
      return res.status(400).json({
        error: "Password must contain uppercase letter ex.(ABCDEF...)",
      });
    }
...
app.post("/user/login", (req, res, next) => {
  ...
  passport.authenticate("local", (err, user, info) => {
    ...
    if (!user) {
      ...
      return res
        .status(400)
        .json({ error: info && info.message ? info.message : "login failed" });
    }
    req.login(user, (err) => {
      if (err) {
  ...
  req.logOut((err) => {
    ...
    req.session.destroy((err) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ error: "Logout failed, could not destroy the session." });
      }
...
app.get("/manga", validateQuery, async (req, res) => {
  ...
  try {
    const result = await sortMangaByFilters(req.validated, {
      page: limit * (page - 1),
      limit: limit,
    ...
    console.log("direction in manga route: ", req.validated.direction);
    res.status(200).json({
      page: cPage,
      totalCount: count,
      pageNum: page,
...
app.delete(
  ...
  async (req, res) => {
    ...
    if (MangaIdError) {
      return res
        .status(MangaIdError.status)
        .json({ error: MangaIdError.message });
    }
...
app.get("/search", async (req, res) => {
  ...
  WITH candidates AS (
    SELECT
      ...
      similarity(search_title_lower, LOWER($1)) AS sim
    FROM manga
    WHERE search_title_lower % LOWER($1)
  )
  SELECT *
  FROM candidates
  WHERE sim > 0.3
...
LIMIT 24
`;
    const result = await db.query(q2, [query]);
...
app.get("/user/user", isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ email: userIdResult.email, username: userIdResult.username });
  } catch (err) {
  ...
  try {
    ...
    if (updates.email) {
      ...
      await db.query("UPDATE users SET email = $1 where id = $2", [
        newEmail,
        userId,
      ]); // Consider adding a regex to validate the format of newEmail
    ...
    if (updates.password) {
      ...
      const passCheck = argon2.verify(user.password, newPlainTextPass, {
        secret: pepper,
      });
      if (passCheck) {
        return res
