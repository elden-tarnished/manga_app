...
async function filteringDBFromIds() {
  const ids = fs
    .readFileSync(__allValidMangaIdsPath, { encoding: "utf-8" })
    .split("\n")
    .filter(Boolean);
  ...
  console.log(
    `all Ids Set: ${goodIds.length}, db Ids Set: ${DBIdsSet.size},,, ${goodIds.length - DBIdsSet.size}\n`,
  );
...
async function readErrorUrls() {
  ...
  try {
    ...
    if (err.response?.status) {
      errorLogger(
        color.dbBrakingError("Critical error on reading Urls: ", err.message),
      );
    }
  }
...
async function WriteDbIds() {
  try {
    const result = await db.query(
      "SELECT id FROM manga WHERE title IS NOT NULL",
    );
...
async function writeMAL_To_DB(url) {
  ...
  try {
    ...
    if (
      ...
      if (combined.size > 0) {
        ...
        const combinedPlaceholder = combinedArray
          ...
          .join(", ");
        await dbClient.query(
          `INSERT INTO manga (id) VALUES ${combinedPlaceholder} ON CONFLICT (id) DO NOTHING`,
          combinedArray,
        );
      }
    }
    ...
    if (alreadyExist) {
      log(
        color.dbSuccess(`insertion was succesful: ${url?.split("/").pop()}\n`),
      );
    } else {
  ...
  while (retries >= 0) {
    try {
      ...
      if (
        ...
        if (retries === 0) {
          errorLogger(
            color.dbBrakingError(`Gave up after ${retries}, ID:${err.id}`),
          );
          mangaErrorsToAppend.push(url);
          return;
        }
...
const failedTxt = fs
  .readFileSync("../texts/failedUrls.txt", { encoding: "utf8" })
  .split("\n");
    ...
    const promise = batchArr[i].map((e) => {
      ...
      return limit(async () => {
        try {
          ...
          errorLogger(
            color.dbBrakingError(
              `Critical error mappig manga url: ${e}, error: ${err}\n`,
            ),
          );
...
async function mangaIdWriter(idWriterUrl, configOffset) {
  try {
    ...
    if (!Array.isArray(result.data.data))
      log("this url aint even a array, check it out!");
...
async function mangaIdPromise(fiveRacks) {
  ...
  const batchArr = Array.from({ length: Math.ceil(fiveRacks / 3) }, (_, i) =>
    batch.slice(i * 3, i * 3 + 3),
  );
  let sleepTime = minute(4.5);
  let sleepAfterBatch = 300;
  let hadTimeout = false;
  ...
  for (let i = 0; i < batchArr.length; i++) {
    log(
      color.dbMutual(
        `batch no: ${i + 1}: ${batchArr[i].at(0)}/${batchArr[i].at(-1) + 500}`,
      ),
    );
    const promise = batchArr[i].map((e) => {
      return limit(async () => {
        try {
          const ids = await mangaIdWriter(
