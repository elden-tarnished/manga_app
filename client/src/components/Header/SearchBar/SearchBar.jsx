...
export function SearchBar() {
  ...
  useEffect(() => {
    ...
    const cardWidth = isMobile ? 147 : 184;
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const message = 'Failed to fetch results, try again later.';
        setError(message);
        console.error(err);
  ...
  function resultContainerHeightAnimate(itemsNumber) {
    ...
    if (itemsNumber > 4) { height = 3 * 98 + 4 * 8; }
    else if (itemsNumber > 2) { height = 2 * 98 + 3 * 8 }
    else { height = 98 + 2 * 8 }
    resultTween()
  }
  ...
  const { contextSafe } = useGSAP(() => {
    tlRef.current = gsap.timeline({ paused: true })
      ...
      .fromTo(resultContainerRef.current, {
        pointerEvents: 'none',
        scale: 0.6,
        width: 0,
        y: -20,
  ...
  }, { dependencies: [rerenderImgs, isMobile] });
