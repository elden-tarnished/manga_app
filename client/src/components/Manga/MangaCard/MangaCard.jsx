...
export function MangaCard(props) {
  ...
  const { contextSafe } = useGSAP(() => {
    ...
    tl2.current.to(detailAndSynopsisRef.current,
      {
    ...
    tlRevert.current = () => {
      ...
      if (tl2.current.time() > pauseTime) {
        tl1.current.timeScale(2).reverse().eventCallback('onUpdate', () => {
        ...
        tl1.current.timeScale(2).reverse()
        tlYoyo.revert();
      }
    }

    tlFirstPlay.current = () => {
      tl1.current.timeScale(1).play()
      tl2.current.timeScale(1).play(0);
      tlYoyo.play(0)
    }
    tlSynopsisReverse.current = () => {
      if (tl2.current.progress() === 1) {
        tl2.current.timeScale(1).reverse().eventCallback('onUpdate', () => {
    ...
    Observer.create({
      ...
      tolerance: 20,
      onRight: () => {
        if (tl1.current.progress() > 0) return;
        tlFirstPlay.current()
      },
      onLeft: () => {
        tlRevert.current()
    ...
    Observer.create({
      ...
      onClick: () => {
        if (tlSynopsis.progress() === 1) {
          if (tl2.current.progress() === 1) {
            tl2.current.timeScale(2).reverse().eventCallback('onUpdate', () => {
              tlSynopsis.timeScale(2).reverse()
            })
          }
        }
        else {
          tl2.current.timeScale(1).play().eventCallback('onComplete', () => {
            if (tl1.current.progress() > 0.5) tlSynopsis.timeScale(1).play()
          })
        }
      },
      onDown: () => {
